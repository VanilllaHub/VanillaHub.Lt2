...
mainStroke.Color = BORDER_COLOR; mainStroke.Thickness = 1.2; mainStroke.Transparency = 0.3

TweenService:Create(wrapper, TweenInfo.new(0.65, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 540, 0, 360)}):Play()
TweenService:Create(main,    TweenInfo.new(0.65, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 540, 0, 360), BackgroundTransparency = 0}):Play()

local topBar = Instance.new("Frame", main)
topBar.Size = UDim2.new(1, 0, 0, 40); topBar.BackgroundColor3 = BG_TOP; topBar.BorderSizePixel = 0; topBar.ZIndex = 4

local topBarSep = Instance.new("Frame", topBar)
topBarSep.Size = UDim2.new(1, 0, 0, 1); topBarSep.Position = UDim2.new(0, 0, 1, -1)
topBarSep.BackgroundColor3 = SEP_COLOR; topBarSep.BorderSizePixel = 0; topBarSep.ZIndex = 5
...
versionLbl.TextColor3 = Color3.fromRGB(130, 130, 130); versionLbl.TextXAlignment = Enum.TextXAlignment.Right; versionLbl.ZIndex = 5
...
side.Size = UDim2.new(0, 155, 1, -40); side.Position = UDim2.new(0, 0, 0, 40)
side.BackgroundColor3 = BG_SIDE; side.BorderSizePixel = 0
side.ScrollBarThickness = 3; side.ScrollBarImageColor3 = Color3.fromRGB(90, 90, 90)
side.CanvasSize = UDim2.new(0, 0, 0, 0); side.ZIndex = 2
...
sideSep.Size = UDim2.new(0, 1, 1, -40); sideSep.Position = UDim2.new(0, 155, 0, 40)
sideSep.BackgroundColor3 = SEP_COLOR; sideSep.BorderSizePixel = 0; sideSep.ZIndex = 3

local content = Instance.new("Frame", main)
content.Size = UDim2.new(1, -156, 1, -40); content.Position = UDim2.new(0, 156, 0, 40)
content.BackgroundColor3 = BG_DARK; content.BorderSizePixel = 0
...
task.spawn(function()
    ...
    wt.Text = "You're back, " .. player.DisplayName .. ". VanillaHub is ready."
...
-- registry for search
local searchRegistry = {}  -- {label, tabName, callback (optional)}

local function registerSearchItem(label, tabName, activateFn)
    table.insert(searchRegistry, {label = string.lower(label), display = label, tabName = tabName, activate = activateFn})
end
...
for _, name in ipairs(tabs) do
    ...
    frame.Name = name; frame.Size = UDim2.new(1, 0, 0, 34)
    frame.BackgroundColor3 = Color3.fromRGB(0, 0, 0); frame.BorderSizePixel = 0
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 7)

    local nameLbl = Instance.new("TextLabel", frame)
    nameLbl.Name = "TabLabel"; nameLbl.Size = UDim2.new(1, -14, 1, 0)
    nameLbl.Position = UDim2.new(0, 10, 0, 0); nameLbl.BackgroundTransparency = 1
    nameLbl.Font = Enum.Font.GothamSemibold; nameLbl.TextSize = 13
    nameLbl.TextColor3 = Color3.fromRGB(110, 110, 110); nameLbl.TextXAlignment = Enum.TextXAlignment.Left
    nameLbl.Text = name

    local btn = Instance.new("TextButton", frame)
    btn.Name = name .. "_Btn"; btn.Size = UDim2.new(1, 0, 1, 0)
    btn.BackgroundTransparency = 1; btn.Text = ""; btn.ZIndex = 10; btn.AutoButtonColor = false
...
local guiOpen = true; local isAnimatingGUI = false; local keybindButtonGUI
...
local function toggleGUI()
    ...
    if guiOpen then
        ...
        wrapper.Size = UDim2.new(0,0,0,0)
        local t = TweenService:Create(main, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0,540,0,360), BackgroundTransparency = 0})
        TweenService:Create(wrapper, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0,540,0,360)}):Play()
        t:Play(); t.Completed:Connect(function() isAnimatingGUI = false end)
    else
        local t = TweenService:Create(main, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Size = UDim2.new(0,0,0,0), BackgroundTransparency = 1})
        TweenService:Create(wrapper, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Size = UDim2.new(0,0,0,0)}):Play()
...
bubbleMsg.Text = "Welcome back, " .. player.DisplayName .. ". Glad you are here. Lets get to it."; bubbleMsg.ZIndex = 3

local statsContainer = Instance.new("Frame", homePage)
statsContainer.Size = UDim2.new(1,0,0,160); statsContainer.BackgroundTransparency = 1; statsContainer.LayoutOrder = 2
...
discordFrame.Size = UDim2.new(1,0,0,44); discordFrame.BackgroundColor3 = Color3.fromRGB(16,16,16)
discordFrame.BorderSizePixel = 0; discordFrame.LayoutOrder = 3
Instance.new("UICorner", discordFrame).CornerRadius = UDim.new(0,10)

local discordLbl = Instance.new("TextLabel", discordFrame)
discordLbl.Size = UDim2.new(1,-130,1,0); discordLbl.Position = UDim2.new(0,14,0,0)
discordLbl.BackgroundTransparency = 1; discordLbl.Font = Enum.Font.GothamSemibold; discordLbl.TextSize = 12
discordLbl.TextColor3 = Color3.fromRGB(120,120,120); discordLbl.TextXAlignment = Enum.TextXAlignment.Left
discordLbl.Text = "discord.gg/bpfjSze8VB"
...
copyBtn.BackgroundColor3 = Color3.fromRGB(28,28,28); copyBtn.BorderSizePixel = 0
copyBtn.Font = Enum.Font.GothamSemibold; copyBtn.TextSize = 12
copyBtn.TextColor3 = Color3.fromRGB(170,170,170); copyBtn.Text = "Copy Invite"
Instance.new("UICorner", copyBtn).CornerRadius = UDim.new(0,7)
copyBtn.MouseEnter:Connect(function() TweenService:Create(copyBtn,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(42,42,42)}):Play() end)
copyBtn.MouseLeave:Connect(function() TweenService:Create(copyBtn,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(28,28,28)}):Play() end)
copyBtn.MouseButton1Click:Connect(function()
    pcall(function() setclipboard("https://discord.gg/bpfjSze8VB") end)
    copyBtn.Text = "Copied"
...
local _serverAgeSnapshot = 0; local _loadClock = os.clock()
...
table.insert(cleanupTasks, function() if uptimeThread then pcall(task.cancel,uptimeThread); uptimeThread=nil end end)
...
lagThread = task.spawn(function()
    while gui and gui.Parent do
        ...
        if lagLabel and lagLabel.Parent then
            if ok then
                if ping > 250 then lagLabel.Text = "Bad Ping"; lagLabel.TextColor3 = Color3.fromRGB(200,200,200)
                elseif ping > 120 then lagLabel.Text = "High Ping"; lagLabel.TextColor3 = Color3.fromRGB(180,180,180)
                else lagLabel.Text = "Good Ping"; lagLabel.TextColor3 = Color3.fromRGB(220,220,220) end
            else lagLabel.Text = "Lag: N/A" end
        end
        task.wait(5)
    end
end)
table.insert(cleanupTasks, function() if lagThread then pcall(task.cancel,lagThread); lagThread=nil end end)
...
local function makeWorldSectionLabel(text)
    local w = Instance.new("Frame", worldPage); w.Size = UDim2.new(1,0,0,24); w.BackgroundTransparency = 1
    local lbl = Instance.new("TextLabel", w)
    lbl.Size = UDim2.new(1,-4,1,0); lbl.Position = UDim2.new(0,4,0,0); lbl.BackgroundTransparency = 1
    lbl.Font = Enum.Font.GothamBold; lbl.TextSize = 10; lbl.TextColor3 = SECTION_TEXT
    lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.Text = "  " .. string.upper(text)
end

local function makeWorldSep()
    local s = Instance.new("Frame", worldPage); s.Size = UDim2.new(1,0,0,1)
    s.BackgroundColor3 = SEP_COLOR; s.BorderSizePixel = 0
end

local function makeWorldToggle(labelText, default, callback)
    local frame = Instance.new("Frame", worldPage); frame.Size = UDim2.new(1,0,0,36)
    ...
    lbl.Text = labelText; lbl.Font = Enum.Font.GothamSemibold; lbl.TextSize = 13
    ...
    tb.BackgroundColor3 = default and SW_ON or SW_OFF; tb.Text = ""; tb.BorderSizePixel = 0
    ...
    local function setState(val)
        toggled = val
        TweenService:Create(tb, TweenInfo.new(0.2,Enum.EasingStyle.Quint), {BackgroundColor3 = val and SW_ON or SW_OFF}):Play()
...
local setDayState; local setNightState
...
local _, _setDay = makeWorldToggle("Always Day", true, function(v)
    ...
    if v then
        ...
        stopDayNight(); Lighting.ClockTime = 14
        dayConn = RunService.Heartbeat:Connect(function() Lighting.ClockTime = 14 end)
    else
        stopDayNight(); Lighting.ClockTime = origClockTime
    end
end)
setDayState = _setDay
...
local _, _setNight = makeWorldToggle("Always Night", false, function(v)
    ...
    if v then
        ...
        stopDayNight(); Lighting.ClockTime = 0
...
makeWorldToggle("Remove Fog", false, function(v)
    if worldFogConn then worldFogConn:Disconnect(); worldFogConn = nil end
    if v then
        Lighting.FogEnd = 1e9; Lighting.FogStart = 1e9
        worldFogConn = RunService.Heartbeat:Connect(function()
            Lighting.FogEnd = 1e9; Lighting.FogStart = 1e9
        end)
    else
        -- restore exactly what was captured on script load
...
makeWorldToggle("Shadows", true, function(v) Lighting.GlobalShadows = v end)

makeWorldSep(); makeWorldSectionLabel("Water")

local walkOnWaterConn = nil; local walkOnWaterParts = {}
    ...
    if v then
        local function makeSolid(part)
            if part:IsA("Part") and part.Name == "Water" then
                ...
                clone.Size = part.Size; clone.CFrame = part.CFrame; clone.Anchored = true
                clone.CanCollide = true; clone.Transparency = 1; clone.Name = "WalkWaterPlane"
                clone.Parent = workspace; table.insert(walkOnWaterParts, clone)
            end
        end
...
makeWorldToggle("Remove Water", false, function(v)
    for _, p in ipairs(workspace:GetDescendants()) do
        if p:IsA("Part") and p.Name == "Water" then
            p.Transparency = v and 1 or 0.5; p.CanCollide = false
        end
    end
end)

makeWorldSep(); makeWorldSectionLabel("World")
...
-- register world toggles for search
registerSearchItem("Always Day",    "WorldTab")
registerSearchItem("Always Night",  "WorldTab")
registerSearchItem("Remove Fog",    "WorldTab")
registerSearchItem("Shadows",       "WorldTab")
registerSearchItem("Walk On Water", "WorldTab")
registerSearchItem("Remove Water",  "WorldTab")
...
local locations = {
    ...
    {name="Links Logic",       x=4605,     y=3,       z=-727},
    ...
    {name="Wood RUs",          x=265,      y=3.2,     z=57},
...
-- clean search bar, no icon
local searchBarFrame = Instance.new("Frame", teleportPage)
searchBarFrame.Size = UDim2.new(1,0,0,32); searchBarFrame.BackgroundColor3 = Color3.fromRGB(16,16,16)
...
searchBox.Size = UDim2.new(1,-16,1,0); searchBox.Position = UDim2.new(0,10,0,0)
searchBox.BackgroundTransparency = 1; searchBox.Font = Enum.Font.Gotham; searchBox.TextSize = 13
searchBox.TextColor3 = THEME_TEXT; searchBox.PlaceholderText = "Search locations..."
searchBox.PlaceholderColor3 = Color3.fromRGB(55,55,55); searchBox.Text = ""
searchBox.ClearTextOnFocus = false; searchBox.TextXAlignment = Enum.TextXAlignment.Left

local chipGrid = Instance.new("Frame", teleportPage)
chipGrid.BackgroundTransparency = 1; chipGrid.Size = UDim2.new(1,0,0,0)

local chipLayout = Instance.new("UIGridLayout", chipGrid)
chipLayout.CellSize = UDim2.new(0.5,-5,0,28); chipLayout.CellPadding = UDim2.new(0,8,0,6)
...
chipLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    chipGrid.Size = UDim2.new(1,0,0,chipLayout.AbsoluteContentSize.Y)
end)

local tpChips = {}

for i, loc in ipairs(locations) do
    local chip = Instance.new("TextButton", chipGrid)
    chip.Name = loc.name; chip.LayoutOrder = i
    chip.BackgroundColor3 = Color3.fromRGB(18,18,18); chip.BorderSizePixel = 0; chip.AutoButtonColor = false
    chip.Font = Enum.Font.GothamSemibold; chip.TextSize = 12
    chip.TextColor3 = Color3.fromRGB(150,150,150); chip.Text = loc.name; chip.TextTruncate = Enum.TextTruncate.AtEnd
    Instance.new("UICorner", chip).CornerRadius = UDim.new(1,0)

    chip.MouseEnter:Connect(function()
        TweenService:Create(chip,TweenInfo.new(0.14),{BackgroundColor3=Color3.fromRGB(32,32,32),TextColor3=Color3.fromRGB(220,220,220)}):Play()
    end)
    chip.MouseLeave:Connect(function()
        TweenService:Create(chip,TweenInfo.new(0.14),{BackgroundColor3=Color3.fromRGB(18,18,18),TextColor3=Color3.fromRGB(150,150,150)}):Play()
    end)
    chip.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(chip,TweenInfo.new(0.07),{BackgroundColor3=Color3.fromRGB(55,55,55)}):Play()
        task.delay(0.15, function()
            TweenService:Create(chip,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(18,18,18)}):Play()
        end)
    end)
    table.insert(tpChips, {chip=chip, name=string.lower(loc.name)})

    -- register for global search
    local locRef = loc
    registerSearchItem(loc.name, "TeleportTab", function()
        local char = player.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
            char.HumanoidRootPart.CFrame = CFrame.new(locRef.x, locRef.y + 3, locRef.z)
        end
    end)
end
...
searchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    local order = 1
    for _, entry in ipairs(tpChips) do
        local visible = query == "" or string.find(entry.name, query, 1, true)
        entry.chip.Visible = visible ~= nil
        if entry.chip.Visible then entry.chip.LayoutOrder = order; order = order + 1 end
    end
end)
...
local clickSelectEnabled = false; local lassoEnabled = false
local groupSelectEnabled = false; local isTeleportingItems = false; local stopTeleportItems = false

local function iSectionLabel(text)
    local w = Instance.new("Frame", itemPage); w.Size = UDim2.new(1,0,0,24); w.BackgroundTransparency = 1
    local lbl = Instance.new("TextLabel", w)
    lbl.Size = UDim2.new(1,-4,1,0); lbl.Position = UDim2.new(0,4,0,0); lbl.BackgroundTransparency = 1
    lbl.Font = Enum.Font.GothamBold; lbl.TextSize = 10; lbl.TextColor3 = SECTION_TEXT
    lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.Text = "  " .. string.upper(text)
...
local function iSep()
    local sep = Instance.new("Frame", itemPage); sep.Size = UDim2.new(1,0,0,1)
...
local function iButton(text, cb)
    ...
    local s = Instance.new("UIStroke", btn); s.Color = Color3.fromRGB(55,55,55); s.Thickness = 1; s.Transparency = 0
...
local function iToggle(text, default, cb)
    local frame = Instance.new("Frame", itemPage); frame.Size = UDim2.new(1,0,0,36)
    frame.BackgroundColor3 = Color3.fromRGB(20,20,20); frame.BorderSizePixel = 0
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0,8)
    local lbl = Instance.new("TextLabel", frame)
    lbl.Size = UDim2.new(1,-54,1,0); lbl.Position = UDim2.new(0,12,0,0); lbl.BackgroundTransparency = 1
    ...
    circle.BackgroundColor3 = default and SW_KNOB_ON or SW_KNOB_OFF; circle.BorderSizePixel = 0
    Instance.new("UICorner", circle).CornerRadius = UDim.new(1,0)
    local toggled = default; if cb then cb(toggled) end
    tb.MouseButton1Click:Connect(function()
        toggled = not toggled
...
local function iSlider(text, minV, maxV, defV, cb)
    local frame = Instance.new("Frame", itemPage); frame.Size = UDim2.new(1,0,0,54)
    frame.BackgroundColor3 = Color3.fromRGB(20,20,20); frame.BorderSizePixel = 0
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0,8)
    ...
    valLbl.Font = Enum.Font.GothamBold; valLbl.TextSize = 13; valLbl.TextColor3 = PB_TEXT
    ...
    fill.Size = UDim2.new((defV-minV)/(maxV-minV),0,1,0); fill.BackgroundColor3 = PB_BAR; fill.BorderSizePixel = 0
    ...
    track.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then ds=true; upd(i.Position.X) end end)
    UserInputService.InputChanged:Connect(function(i) if ds and i.UserInputType==Enum.UserInputType.MouseMovement then upd(i.Position.X) end end)
...
local function selectPart(part)
    ...
    sb.SurfaceColor3 = Color3.fromRGB(0,0,0); sb.Color3 = Color3.fromRGB(180,180,180)
end
...
local function trySelect(target)
    ...
    if par:FindFirstChild("Main") then
        ...
        if target == tPart or target:IsDescendantOf(tPart) then
            if tPart:FindFirstChild("Selection") then deselectPart(tPart) else selectPart(tPart) end; return
        end
    end
...
local lassoFrame = Instance.new("Frame", gui); lassoFrame.Name = "VHLassoRect"
lassoFrame.BackgroundColor3 = Color3.fromRGB(100,100,100); lassoFrame.BackgroundTransparency = 0.82
...
UserInputService.InputBegan:Connect(function(input)
    ...
    if not workspace:FindFirstChild("PlayerModels") then return end
    lassoFrame.Visible = true; lassoFrame.Position = UDim2.new(0,mouse.X,0,mouse.Y); lassoFrame.Size = UDim2.new(0,0,0,0)
...
iToggle("Click Selection", false, function(val)
    clickSelectEnabled = val; if val then lassoEnabled=false; groupSelectEnabled=false end
end)
iToggle("Lasso Tool", false, function(val)
    lassoEnabled = val; if val then clickSelectEnabled=false; groupSelectEnabled=false end
end)
iToggle("Group Selection", false, function(val)
    groupSelectEnabled = val; if val then clickSelectEnabled=false; lassoEnabled=false end
end)

iSep(); iSectionLabel("Delay Per Item")
iSlider("Delay (x0.1s)", 1, 20, 3, function(v) tpItemSpeed = v/10 end)

iSep(); iSectionLabel("Teleport Mode")

local itemModeRow = Instance.new("Frame", itemPage)
itemModeRow.Size = UDim2.new(1,0,0,30); itemModeRow.BackgroundTransparency = 1
local itemModeButtons = {}; local itemModeNames = {"Group","Random"}; local itemTpMode = "group"
...
for i, mName in ipairs(itemModeNames) do
    ...
    mb.Size = UDim2.new(0.5,-4,1,0); mb.Position = UDim2.new((i-1)*0.5,i==1 and 0 or 4,0,0)
    ...
    Instance.new("UICorner", mb).CornerRadius = UDim.new(0,7); table.insert(itemModeButtons, mb)
    mb.MouseButton1Click:Connect(function() itemTpMode=string.lower(mName); updateItemModeButtons(mName) end)
end
...
iSep(); iSectionLabel("Teleport Destination")

local tpRow = Instance.new("Frame", itemPage); tpRow.Size = UDim2.new(1,0,0,30); tpRow.BackgroundTransparency = 1
...
tpSetBtn.BorderSizePixel = 0; Instance.new("UICorner", tpSetBtn).CornerRadius = UDim.new(0,7)
local s1 = Instance.new("UIStroke", tpSetBtn); s1.Color = Color3.fromRGB(55,55,55); s1.Thickness = 1; s1.Transparency = 0
...
tpRemoveBtn.BorderSizePixel = 0; Instance.new("UICorner", tpRemoveBtn).CornerRadius = UDim.new(0,7)
local s2 = Instance.new("UIStroke", tpRemoveBtn); s2.Color = Color3.fromRGB(55,55,55); s2.Thickness = 1; s2.Transparency = 0
...
tpSetBtn.MouseButton1Click:Connect(function()
    ...
    tpCircle.Material = Enum.Material.SmoothPlastic; tpCircle.Color = Color3.fromRGB(130,130,130)
...
tpSelectBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        if not workspace:FindFirstChild("PlayerModels") then
            isTeleportingItems=false; stopTeleportItems=false; tpSelectBtn.Text="Teleport Selected"
            TweenService:Create(tpSelectBtn,TweenInfo.new(0.2),{BackgroundColor3=BTN_COLOR}):Play(); return
        end
        local selectedParts = {}
        for _, v in next, workspace.PlayerModels:GetDescendants() do
            if v.Name == "Selection" then
                local part = v.Parent; if part and part.Parent then table.insert(selectedParts, part) end
            end
        end
        local function getItemType(part)
            local m = part.Parent; if not m then return "unknown" end
            local iv = m:FindFirstChild("ItemName"); return iv and iv.Value or m.Name
        end
        if itemTpMode == "random" then
            for i = #selectedParts, 2, -1 do
                local j = math.random(i); selectedParts[i],selectedParts[j] = selectedParts[j],selectedParts[i]
            end
        elseif itemTpMode == "group" then
            table.sort(selectedParts, function(a,b) return getItemType(a) < getItemType(b) end)
        end
        for _, part in ipairs(selectedParts) do
            ...
            task.wait(tpItemSpeed); if stopTeleportItems then break end
            pcall(function()
                if not part.Parent.PrimaryPart then part.Parent.PrimaryPart = part end
                local dragger = ReplicatedStorage:FindFirstChild("Interaction") and ReplicatedStorage.Interaction:FindFirstChild("ClientIsDragging")
                local timeout = 0
                while not isnetworkowner(part) and timeout < 3 do
                    if dragger then dragger:FireServer(part.Parent) end; task.wait(0.05); timeout = timeout+0.05
                end
...
sellBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        for _, v in next, workspace.PlayerModels:GetDescendants() do
            if v.Name == "Selection" then
                local part = v.Parent; if not (part and part.Parent) then continue end
                local char = player.Character; local hrp = char and char:FindFirstChild("HumanoidRootPart")
                if hrp then hrp.CFrame = CFrame.new(part.CFrame.p)*CFrame.new(5,0,0) end
                task.wait(tpItemSpeed)
                pcall(function()
...
registerSearchItem("Click Selection",  "ItemTab")
registerSearchItem("Lasso Tool",       "ItemTab")
registerSearchItem("Group Selection",  "ItemTab")
registerSearchItem("Teleport Selected","ItemTab")
registerSearchItem("Sell Selected",    "ItemTab")
registerSearchItem("Deselect All",     "ItemTab")
...
local function dSectionLabel(text)
    local w = Instance.new("Frame", dupePage); w.Size = UDim2.new(1,0,0,22); w.BackgroundTransparency = 1
    local lbl = Instance.new("TextLabel", w)
    lbl.Size = UDim2.new(1,-4,1,0); lbl.Position = UDim2.new(0,4,0,0); lbl.BackgroundTransparency = 1
    lbl.Font = Enum.Font.GothamBold; lbl.TextSize = 10; lbl.TextColor3 = SECTION_TEXT
    lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.Text = "  " .. string.upper(text)
...
local function createPSection(text)
    local w = Instance.new("Frame", playerPage); w.Size = UDim2.new(1,0,0,24); w.BackgroundTransparency = 1
    local lbl = Instance.new("TextLabel", w)
    lbl.Size = UDim2.new(1,-4,1,0); lbl.Position = UDim2.new(0,4,0,0); lbl.BackgroundTransparency = 1
    lbl.Font = Enum.Font.GothamBold; lbl.TextSize = 10; lbl.TextColor3 = SECTION_TEXT
    lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.Text = "  " .. string.upper(text)
...
local function createPSep()
    local s = Instance.new("Frame", playerPage); s.Size = UDim2.new(1,0,0,1)
    s.BackgroundColor3 = SEP_COLOR; s.BorderSizePixel = 0
end

local savedWalkSpeed = 16; local savedJumpPower = 50
...
local function createPSlider(labelText, minVal, maxVal, defaultVal, onChanged)
    local frame = Instance.new("Frame", playerPage); frame.Size = UDim2.new(1,0,0,54)
    frame.BackgroundColor3 = Color3.fromRGB(20,20,20); frame.BorderSizePixel = 0
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0,8)
    local topRow = Instance.new("Frame", frame)
    topRow.Size = UDim2.new(1,-16,0,22); topRow.Position = UDim2.new(0,8,0,7); topRow.BackgroundTransparency = 1
    ...
    Instance.new("UICorner", track).CornerRadius = UDim.new(1,0)
    local fill = Instance.new("Frame", track)
    fill.Size = UDim2.new((defaultVal-minVal)/(maxVal-minVal),0,1,0); fill.BackgroundColor3 = PB_BAR; fill.BorderSizePixel = 0
    Instance.new("UICorner", fill).CornerRadius = UDim.new(1,0)
    local knob = Instance.new("TextButton", track)
    knob.Size = UDim2.new(0,14,0,14); knob.AnchorPoint = Vector2.new(0.5,0.5)
...
local function createPToggle(text, defaultState, callback)
    local frame = Instance.new("Frame", playerPage); frame.Size = UDim2.new(1,0,0,36)
    frame.BackgroundColor3 = Color3.fromRGB(20,20,20); frame.BorderSizePixel = 0
    Instance.new("UICorner", frame).CornerRadius = UDim.new(0,8)
    local lbl = Instance.new("TextLabel", frame)
    lbl.Size = UDim2.new(1,-54,1,0); lbl.Position = UDim2.new(0,12,0,0); lbl.BackgroundTransparency = 1
    ...
    local tb = Instance.new("TextButton", frame)
    tb.Size = UDim2.new(0,36,0,20); tb.Position = UDim2.new(1,-46,0.5,-10)
    tb.BackgroundColor3 = defaultState and SW_ON or SW_OFF; tb.Text = ""; tb.BorderSizePixel = 0
    Instance.new("UICorner", tb).CornerRadius = UDim.new(1,0)
    local circle = Instance.new("Frame", tb)
    ...
    Instance.new("UICorner", circle).CornerRadius = UDim.new(1,0)
    local toggled = defaultState; if callback then callback(toggled) end
    ...
    tb.MouseButton1Click:Connect(function() toggled=not toggled; setToggled(toggled); if callback then callback(toggled) end end)
...
createPSlider("Walkspeed", 16, 150, 16, function(val)
    savedWalkSpeed = val
    local char = player.Character; if char and char:FindFirstChild("Humanoid") then char.Humanoid.WalkSpeed = val end
end)
createPSlider("Jumppower", 50, 300, 50, function(val)
    savedJumpPower = val
    local char = player.Character; if char and char:FindFirstChild("Humanoid") then char.Humanoid.JumpPower = val end
end)

registerSearchItem("Walkspeed",  "PlayerTab")
registerSearchItem("Jumppower",  "PlayerTab")

local flySpeed = 100; local flyEnabled = true; local isFlyActive = false
local flyBV, flyBG, flyConn; local currentFlyKey = Enum.KeyCode.Q

local function stopFly()
    isFlyActive = false; if _G.VH then _G.VH.isFlyActive = false end
...
local function startFly()
    if not flyEnabled then return end; stopFly()
    local char = player.Character; if not char then return end
    local root = char:FindFirstChild("HumanoidRootPart"); local hum = char:FindFirstChild("Humanoid")
    if not root or not hum then return end
    isFlyActive = true; if _G.VH then _G.VH.isFlyActive = true end
    ...
    flyConn = RunService.Heartbeat:Connect(function()
        ...
        local cf = workspace.CurrentCamera.CFrame; local dir = Vector3.zero; local UIS = UserInputService
        ...
        flyBG.MaxTorque = Vector3.new(1e6,1e6,1e6); flyBG.CFrame = cf
    end)
end
...
registerSearchItem("Fly Speed", "PlayerTab")

local flyKeyFrame = Instance.new("Frame", playerPage); flyKeyFrame.Size = UDim2.new(1,0,0,36)
...
flyKeyLabel.TextColor3 = THEME_TEXT; flyKeyLabel.TextXAlignment = Enum.TextXAlignment.Left; flyKeyLabel.Text = "Fly Hotkey"
...
local fkS = Instance.new("UIStroke", flyKeyBtn); fkS.Color = Color3.fromRGB(55,55,55); fkS.Thickness = 1; fkS.Transparency = 0
...
flyKeyBtn.MouseButton1Click:Connect(function()
    ...
    waitingForFlyKey = true; flyKeyBtn.Text = "..."; flyKeyBtn.BackgroundColor3 = Color3.fromRGB(60,60,60)
end)

local flyToggleFrame = Instance.new("Frame", playerPage); flyToggleFrame.Size = UDim2.new(1,0,0,36)
...
flyToggleTb.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(flyToggleTb,TweenInfo.new(0.2,Enum.EasingStyle.Quint),{BackgroundColor3=flyEnabled and SW_ON or SW_OFF}):Play()
...
registerSearchItem("Fly",        "PlayerTab")
registerSearchItem("Fly Hotkey", "PlayerTab")

createPSep(); createPSection("Character")
...
table.insert(cleanupTasks, function() noclipEnabled=false; if noclipConn then noclipConn:Disconnect(); noclipConn=nil end end)
...
table.insert(cleanupTasks, function() infJumpEnabled=false; if infJumpConn then infJumpConn:Disconnect(); infJumpConn=nil end end)

registerSearchItem("Noclip",  "PlayerTab")
registerSearchItem("InfJump", "PlayerTab")

createPSep(); createPSection("Misc")

local hardDragEnabled = false; local draggerConn = nil
...
local function startDraggerWatch()
    ...
    draggerConn = workspace.ChildAdded:Connect(function(a)
        ...
        local bg = a:WaitForChild("BodyGyro",2); local bp = a:WaitForChild("BodyPosition",2)
        if not (bg and bp) then return end
        task.spawn(function()
            while a and a.Parent do
                if hardDragEnabled then
                    bp.P=120000; bp.D=1000; bp.maxForce=Vector3.new(math.huge,math.huge,math.huge)
                    bg.maxTorque=Vector3.new(math.huge,math.huge,math.huge)
                else
                    bp.P=10000; bp.D=800; bp.maxForce=Vector3.new(17000,17000,17000)
...
createPToggle("Hard Dragger", false, function(val)
    ...
    if not val then
        ...
        if d then
            local bp = d:FindFirstChild("BodyPosition"); local bg = d:FindFirstChild("BodyGyro")
...
registerSearchItem("Hard Dragger", "PlayerTab")
...
local globalSearchBar = Instance.new("Frame", searchPage)
globalSearchBar.Size = UDim2.new(1,0,0,32); globalSearchBar.BackgroundColor3 = Color3.fromRGB(16,16,16)
globalSearchBar.BorderSizePixel = 0
Instance.new("UICorner", globalSearchBar).CornerRadius = UDim.new(1,0)

local globalSearchBox = Instance.new("TextBox", globalSearchBar)
globalSearchBox.Size = UDim2.new(1,-16,1,0); globalSearchBox.Position = UDim2.new(0,10,0,0)
globalSearchBox.BackgroundTransparency = 1; globalSearchBox.Font = Enum.Font.Gotham; globalSearchBox.TextSize = 13
globalSearchBox.TextColor3 = THEME_TEXT; globalSearchBox.PlaceholderText = "Search all features..."
globalSearchBox.PlaceholderColor3 = Color3.fromRGB(55,55,55); globalSearchBox.Text = ""
globalSearchBox.ClearTextOnFocus = false; globalSearchBox.TextXAlignment = Enum.TextXAlignment.Left

local searchHint = Instance.new("TextLabel", searchPage)
searchHint.Size = UDim2.new(1,0,0,20); searchHint.BackgroundTransparency = 1
searchHint.Font = Enum.Font.Gotham; searchHint.TextSize = 11
searchHint.TextColor3 = Color3.fromRGB(60,60,60); searchHint.TextXAlignment = Enum.TextXAlignment.Left
searchHint.Text = "  Type to search across all tabs. Click a result to jump to it."

local resultsContainer = Instance.new("Frame", searchPage)
resultsContainer.BackgroundTransparency = 1; resultsContainer.Size = UDim2.new(1,0,0,0)

local resultsLayout = Instance.new("UIListLayout", resultsContainer)
resultsLayout.Padding = UDim.new(0,5); resultsLayout.SortOrder = Enum.SortOrder.LayoutOrder
...
local function clearResults()
    for _, item in ipairs(resultItems) do if item and item.Parent then item:Destroy() end end
    resultItems = {}
end
...
local function buildResults(query)
    ...
    for _, entry in ipairs(searchRegistry) do
        if string.find(entry.label, lq, 1, true) and count < 20 then
            count = count + 1
            local row = Instance.new("Frame", resultsContainer)
            row.Size = UDim2.new(1,0,0,36); row.BackgroundColor3 = Color3.fromRGB(16,16,16)
            row.BorderSizePixel = 0; row.LayoutOrder = count
            ...
            nameLbl.Text = entry.display

            local tabLbl = Instance.new("TextLabel", row)
            tabLbl.Size = UDim2.new(0,80,1,0); tabLbl.Position = UDim2.new(1,-88,0,0)
            tabLbl.BackgroundTransparency = 1; tabLbl.Font = Enum.Font.Gotham; tabLbl.TextSize = 10
            tabLbl.TextColor3 = Color3.fromRGB(80,80,80); tabLbl.TextXAlignment = Enum.TextXAlignment.Right
            tabLbl.Text = entry.tabName:gsub("Tab","")
            ...
            clickBtn.MouseEnter:Connect(function()
                TweenService:Create(row,TweenInfo.new(0.14),{BackgroundColor3=Color3.fromRGB(28,28,28)}):Play()
            end)
            clickBtn.MouseLeave:Connect(function()
                TweenService:Create(row,TweenInfo.new(0.14),{BackgroundColor3=Color3.fromRGB(16,16,16)}):Play()
            end)

            local entryRef = entry
            clickBtn.MouseButton1Click:Connect(function()
                switchTab(entryRef.tabName)
                if entryRef.activate then entryRef.activate() end
            end)

            table.insert(resultItems, row)
        end
    end
    if count == 0 then
        local none = Instance.new("TextLabel", resultsContainer)
        none.Size = UDim2.new(1,0,0,30); none.BackgroundTransparency = 1
        none.Font = Enum.Font.Gotham; none.TextSize = 12
        none.TextColor3 = Color3.fromRGB(70,70,70); none.TextXAlignment = Enum.TextXAlignment.Center
        none.Text = "No results found."; none.LayoutOrder = 1
        table.insert(resultItems, none)
    end
end

globalSearchBox:GetPropertyChangedSignal("Text"):Connect(function()
    buildResults(globalSearchBox.Text)
end)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if waitingForFlyKey then
        if input.UserInputType == Enum.UserInputType.Keyboard then
            currentFlyKey = input.KeyCode; if _G.VH then _G.VH.currentFlyKey = currentFlyKey end
            flyKeyBtn.Text = input.KeyCode.Name; flyKeyBtn.BackgroundColor3 = BTN_COLOR; waitingForFlyKey = false
        end; return
    end
    if gameProcessed then return end
    if input.KeyCode == currentToggleKey then toggleGUI(); return end
    if input.KeyCode == currentFlyKey then
        if not flyEnabled then if isFlyActive then stopFly() end; return end
        if isFlyActive then stopFly() else startFly() end; return
    end
end)
...
_G.VH = {
    ...
    BTN_COLOR=BTN_COLOR, BTN_HOVER=BTN_HOVER, THEME_TEXT=THEME_TEXT, ACCENT=ACCENT,
    switchTab=switchTab, toggleGUI=toggleGUI, stopFly=stopFly, startFly=startFly,
    butter={running=false,thread=nil}, isFlyActive=false, flyEnabled=flyEnabled,
    currentFlyKey=currentFlyKey, waitingForFlyKey=false, flyKeyBtn=flyKeyBtn,
    currentToggleKey=currentToggleKey, keybindButtonGUI=nil,
}
