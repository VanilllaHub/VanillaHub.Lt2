...
local dayTb, dayCir, nightTb, nightCir

local function setToggleVisual(tb, cir, state)
    ...
    TweenService:Create(cir, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {
...
local function makeWorldToggle(labelText)
...
dayTb, dayCir = makeWorldToggle("Always Day")
dayTb.MouseButton1Click:Connect(function()
    alwaysDayEnabled = not alwaysDayEnabled
    if alwaysDayEnabled then
        -- turn off night
        if alwaysNightEnabled then
            alwaysNightEnabled = false
            setToggleVisual(nightTb, nightCir, false)
        end
        stopTimeThread()
    ...
    setToggleVisual(dayTb, dayCir, alwaysDayEnabled)
end)

nightTb, nightCir = makeWorldToggle("Always Night")
nightTb.MouseButton1Click:Connect(function()
    alwaysNightEnabled = not alwaysNightEnabled
    if alwaysNightEnabled then
        -- turn off day
        if alwaysDayEnabled then
            alwaysDayEnabled = false
            setToggleVisual(dayTb, dayCir, false)
        end
        stopTimeThread()
        timeThread = task.spawn(function()
    ...
    setToggleVisual(nightTb, nightCir, alwaysNightEnabled)
end)
