...
-- ════════════════════════════════════════════════════
-- TABS  (Search tab removed)
-- ════════════════════════════════════════════════════
local tabs = {"Home","Player","World","Teleport","Wood","Slot","Dupe","Item","Sorter","AutoBuy","Pixel Art","Build","Vehicle","Settings"}
local pages = {}
...
-- LT2 fog defaults (barely visible, grey, far away)
local LT2_FOG_END   = 100000
local LT2_FOG_START = 2000
local LT2_FOG_COLOR = Color3.fromRGB(190, 190, 190)

local dayConn=nil; local nightConn=nil; local fogConn=nil
local alwaysDayActive=true; local alwaysNightActive=false
...
local _,_setDay=makeWorldToggle("Always Day",true,function(v)
    ...
    else stopDayNight(); Lighting.ClockTime=origClockTime end
end)
setDayState=_setDay
...
makeWorldToggle("Remove Fog",false,function(v)
    ...
    if v then
        ...
        fogConn=RunService.Heartbeat:Connect(function() Lighting.FogEnd=1e9; Lighting.FogStart=1e9 end)
    else
...
-- ════════════════════════════════════════════════════
-- TELEPORT TAB  (pill chip grid, no coords, no emojis)
-- ════════════════════════════════════════════════════
...
searchBarFrame.BackgroundColor3 = Color3.fromRGB(16, 16, 16)
...
searchIcon.Size = UDim2.new(0, 28, 1, 0); searchIcon.Position = UDim2.new(0, 6, 0, 0)
searchIcon.BackgroundTransparency = 1; searchIcon.Text = "⌕"; searchIcon.TextSize = 16
searchIcon.Font = Enum.Font.GothamBold; searchIcon.TextColor3 = Color3.fromRGB(70, 70, 70)

local searchBox = Instance.new("TextBox", searchBarFrame)
searchBox.Size = UDim2.new(1, -36, 1, 0); searchBox.Position = UDim2.new(0, 28, 0, 0)
...
chipLayout.CellSize = UDim2.new(0.5, -5, 0, 28)
chipLayout.CellPadding = UDim2.new(0, 8, 0, 6)
...
for i, loc in ipairs(locations) do
    ...
    chip.TextColor3 = Color3.fromRGB(150, 150, 150)
    chip.Text = loc.name; chip.TextTruncate = Enum.TextTruncate.AtEnd
    Instance.new("UICorner", chip).CornerRadius = UDim.new(1, 0)

    chip.MouseEnter:Connect(function()
        TweenService:Create(chip, TweenInfo.new(0.14), {
...
tpSelectBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if itemTpMode=="random" then
            for i=#selectedParts,2,-1 do local j=math.random(i); selectedParts[i],selectedParts[j]=selectedParts[j],selectedParts[i] end
                ...
                if getItemType(selectedParts[i])==getItemType(selectedParts[i-1]) then
                    for j=i+1,#selectedParts do
                        if getItemType(selectedParts[j])~=getItemType(selectedParts[i-1]) then
                            selectedParts[i],selectedParts[j]=selectedParts[j],selectedParts[i]; break
                        end
                    end
                end
            end
