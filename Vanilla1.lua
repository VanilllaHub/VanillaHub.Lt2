...
-- FIX: wrapper is a transparent holder — no background color, purely for sizing/positioning
...
topBar.BackgroundColor3 = Color3.fromRGB(14, 10, 16)
topBar.BorderSizePixel = 0
topBar.ZIndex = 4

-- Bottom border on topBar (separator line done via UIStroke on a thin frame, no white line)
local topBarSep = Instance.new("Frame", topBar)
topBarSep.Size = UDim2.new(1, 0, 0, 1)
topBarSep.Position = UDim2.new(0, 0, 1, -1)
topBarSep.BackgroundColor3 = Color3.fromRGB(40, 30, 45)
...
closeBtn.BackgroundColor3 = Color3.fromRGB(170, 38, 38); closeBtn.Text = "×"
...
side.BackgroundColor3 = Color3.fromRGB(11, 8, 13)
side.BorderSizePixel = 0
side.ScrollBarThickness = 3
side.ScrollBarImageColor3 = Color3.fromRGB(55, 45, 60)
...
for _, name in ipairs(tabs) do
    ...
    page.ScrollBarThickness = 4; page.ScrollBarImageColor3 = Color3.fromRGB(60, 50, 70)
...
local function switchTab(targetName)
    ...
    if activeTabButton then
        TweenService:Create(activeTabButton, TweenInfo.new(0.22), {
            ...
            TextColor3 = Color3.fromRGB(140, 125, 148)
        }):Play()
    end
    ...
    if btn then
        ...
        TweenService:Create(btn, TweenInfo.new(0.22), {
            BackgroundColor3 = Color3.fromRGB(50, 35, 58),
...
for _, name in ipairs(tabs) do
    ...
    btn.Name = name
    btn.Size = UDim2.new(1, 0, 0, 34)  -- slightly taller tabs
    btn.BackgroundColor3 = Color3.fromRGB(18, 14, 20)
    btn.BorderSizePixel = 0
    btn.Text = name
    btn.Font = Enum.Font.GothamSemibold
    btn.TextSize = 13
    btn.TextColor3 = Color3.fromRGB(140, 125, 148)
    ...
    btn.MouseEnter:Connect(function()
        if activeTabButton ~= btn then
            TweenService:Create(btn, TweenInfo.new(0.18), {
                BackgroundColor3 = Color3.fromRGB(32, 24, 38),
                TextColor3 = Color3.fromRGB(195, 178, 202)
            }):Play()
        end
    end)
...
-- ════════════════════════════════════════════════════
-- GUI TOGGLE  (FIX: hide wrapper background + main)
-- ════════════════════════════════════════════════════
...
local function createStatusBox(text, color)
    ...
    box.BackgroundColor3=Color3.fromRGB(18,14,22); box.BorderSizePixel=0
...
rejoinBtn.Size=UDim2.new(0,148,0,42); rejoinBtn.BackgroundColor3=Color3.fromRGB(18,14,22); rejoinBtn.BorderSizePixel=0
rejoinBtn.Font=Enum.Font.Gotham; rejoinBtn.TextSize=14; rejoinBtn.TextColor3=THEME_TEXT; rejoinBtn.Text="Rejoin"
Instance.new("UICorner",rejoinBtn).CornerRadius=UDim.new(0,8)
rejoinBtn.MouseEnter:Connect(function() TweenService:Create(rejoinBtn,TweenInfo.new(0.18),{BackgroundColor3=Color3.fromRGB(32,24,38)}):Play() end)
rejoinBtn.MouseLeave:Connect(function() TweenService:Create(rejoinBtn,TweenInfo.new(0.18),{BackgroundColor3=Color3.fromRGB(18,14,22)}):Play() end)
...
for _, loc in ipairs(locations) do
    ...
    btn.Size=UDim2.new(1,-12,0,36); btn.BackgroundColor3=Color3.fromRGB(18,14,22)
    ...
    btn.MouseEnter:Connect(function() TweenService:Create(btn,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(38,28,48),TextColor3=Color3.fromRGB(255,255,255)}):Play() end)
    btn.MouseLeave:Connect(function() TweenService:Create(btn,TweenInfo.new(0.15),{BackgroundColor3=Color3.fromRGB(18,14,22),TextColor3=THEME_TEXT}):Play() end)
    btn.MouseButton1Click:Connect(function()
        local char=player.Character
...
local BTN_COLOR = Color3.fromRGB(38, 30, 45)
local BTN_HOVER = Color3.fromRGB(62, 50, 72)
local ACCENT    = Color3.fromRGB(110, 80, 130)
...
local function iSectionLabel(text)
    local wrapper2 = Instance.new("Frame", itemPage)
    wrapper2.Size = UDim2.new(1, 0, 0, 24)
    ...
    lbl.TextSize = 10
    lbl.TextColor3 = Color3.fromRGB(130, 110, 150)
...
local function iSep()
    ...
    sep.BackgroundColor3 = Color3.fromRGB(38, 28, 48)
...
local function iToggle(text, default, cb)
    ...
    frame.BackgroundColor3 = Color3.fromRGB(20, 16, 24)
    frame.BorderSizePixel = 0
    ...
    tb.BackgroundColor3 = default and Color3.fromRGB(80, 160, 80) or Color3.fromRGB(45, 38, 55)
    ...
    tb.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(tb, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {
            BackgroundColor3 = toggled and Color3.fromRGB(80,160,80) or Color3.fromRGB(45,38,55)
        }):Play()
...
local function iSlider(text, minV, maxV, defV, cb)
    ...
    knob.BackgroundColor3 = Color3.fromRGB(220, 205, 235)
...
lassoFrame.BackgroundColor3 = Color3.fromRGB(80, 120, 200)
...
lassoStroke.Color = Color3.fromRGB(120,165,255); lassoStroke.Thickness = 1.5; lassoStroke.Transparency = 0
...
tpRemoveBtn.Size = UDim2.new(0.5, -5, 1, 0)
tpRemoveBtn.Position = UDim2.new(0.5, 5, 0, 0)
tpRemoveBtn.BackgroundColor3 = BTN_COLOR
tpRemoveBtn.Font = Enum.Font.GothamSemibold
tpRemoveBtn.TextSize = 12
tpRemoveBtn.TextColor3 = THEME_TEXT
tpRemoveBtn.Text = "Remove Destination"
...
-- Teleport Selected button with Stop support
...
tpSelectBtn.MouseButton1Click:Connect(function()
    if isTeleportingItems then
        -- Stop teleporting
        stopTeleportItems = true
        return
    end
    if not tpCircle then return end

    isTeleportingItems = true
    stopTeleportItems = false
    tpSelectBtn.Text = "Stop Teleporting"
    TweenService:Create(tpSelectBtn, TweenInfo.new(0.2), {
...
-- ════════════════════════════════════════════════════
-- DUPE TAB — with Random and Group teleport modes
-- ════════════════════════════════════════════════════
...
local dupeTpMode = "normal" -- "normal", "random", "group"
...
local function dSectionLabel(text)
    ...
    lbl.TextColor3 = Color3.fromRGB(130, 110, 150); lbl.TextXAlignment = Enum.TextXAlignment.Left
    lbl.Text = "  " .. string.upper(text)
end
...
dupeModeHint.Size = UDim2.new(1, 0, 0, 34)
dupeModeHint.BackgroundColor3 = Color3.fromRGB(18, 14, 24)
dupeModeHint.BorderSizePixel = 0
dupeModeHint.Font = Enum.Font.Gotham; dupeModeHint.TextSize = 12
dupeModeHint.TextColor3 = Color3.fromRGB(130, 115, 145)
dupeModeHint.TextWrapped = true; dupeModeHint.TextXAlignment = Enum.TextXAlignment.Left
dupeModeHint.Text = "  Normal: sequential  •  Random: shuffled, no consecutive same type  •  Group: by item type"
...
dupeTpBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(dupeTpBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(160,45,45)}):Play()
    ...
    task.spawn(function()
        ...
        if dupeTpMode == "random" then
            -- Fisher-Yates shuffle ensuring no two consecutive same item type
            ...
            -- Ensure no consecutive same type
        ...
        elseif dupeTpMode == "group" then
            -- Sort by item type so same types are grouped together
            table.sort(selectedParts, function(a, b)
                local ma = a.Parent; local mb2 = b.Parent
                local ia = ma and (ma:FindFirstChild("ItemName") and ma.ItemName.Value or ma.Name) or "z"
                local ib = mb2 and (mb2:FindFirstChild("ItemName") and mb2.ItemName.Value or mb2.Name) or "z"
                return ia < ib
            end)
        end
        -- else normal: sequential order as found
...
local function createPSep()
    ...
    s.BackgroundColor3 = Color3.fromRGB(38, 28, 48)
    s.BorderSizePixel = 0
end
...
local function createPSlider(labelText, minVal, maxVal, defaultVal, onChanged)
    ...
    frame.Size=UDim2.new(1,0,0,54); frame.BackgroundColor3=Color3.fromRGB(20,16,24)
    ...
    track.BackgroundColor3=Color3.fromRGB(38,30,48); track.BorderSizePixel=0
    Instance.new("UICorner",track).CornerRadius=UDim.new(1,0)
    local fill=Instance.new("Frame",track)
    ...
    knob.BackgroundColor3=Color3.fromRGB(220,205,235); knob.Text=""; knob.BorderSizePixel=0
    Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
    local ds=false
    local function upd(absX)
        local r=math.clamp((absX-track.AbsolutePosition.X)/track.AbsoluteSize.X,0,1)
...
local function createPToggle(text, defaultState, callback)
    local frame=Instance.new("Frame",playerPage)
    frame.Size=UDim2.new(1,0,0,36); frame.BackgroundColor3=Color3.fromRGB(20,16,24)
    frame.BorderSizePixel=0
    Instance.new("UICorner",frame).CornerRadius=UDim.new(0,8)
    local lbl=Instance.new("TextLabel",frame)
    ...
    tb.BackgroundColor3=defaultState and Color3.fromRGB(80,160,80) or Color3.fromRGB(45,38,55)
    tb.Text=""; tb.BorderSizePixel=0
    Instance.new("UICorner",tb).CornerRadius=UDim.new(1,0)
    local circle=Instance.new("Frame",tb)
    ...
    local function setToggled(val)
        toggled=val
        TweenService:Create(tb,TweenInfo.new(0.2,Enum.EasingStyle.Quint),{BackgroundColor3=toggled and Color3.fromRGB(80,160,80) or Color3.fromRGB(45,38,55)}):Play()
...
-- ════════════════════════════════════════════════════
-- GLOBAL KEY LISTENER (Fly toggle + GUI toggle)
-- Fly: only works when flyEnabled = true
-- ════════════════════════════════════════════════════
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    -- FLY TOGGLE — only if flyEnabled is true
...
print("[VanillaHub] Vanilla1 loaded — Updated")
