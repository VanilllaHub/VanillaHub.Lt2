...
local function setProgress(cur, total)
    if AB_progLbl and AB_progLbl.Parent then
        if cur and total then AB_progLbl.Text = cur .. " / " .. total; AB_progLbl.Visible = true
        else AB_progLbl.Visible = false end
    end
end
...
local function getPrice(itemName)
    ...
    pcall(function()
        for _, v in next, RS:WaitForChild("ClientItemInfo", 5):GetDescendants() do
            if v.Name == itemName and v:FindFirstChild("Price") then price = v.Price.Value; break end
        end
    end)
    return price
end
        ...
        for _, store in next, workspace.Stores:GetChildren() do
            ...
            for _, item in next, store:GetChildren() do
                ...
                if bin and not seen[bin.Value] then
                    seen[bin.Value] = true
                    table.insert(list, { name=bin.Value, price=getPrice(bin.Value), isBlueprint=typ and typ.Value=="Blueprint" })
                end
            end
        end
        ...
        for _, store in next, workspace.Stores:GetChildren() do
            ...
            for _, item in next, store:GetChildren() do
                ...
                if bin and typ and typ.Value == "Blueprint" and not seen[bin.Value] then
                    seen[bin.Value] = true; table.insert(list, bin.Value)
                end
            end
        end
    end)
...
local function fireDialog(c)
    ...
    if c.useWorkspaceRef then
        ...
        if c.preSeq == "SetChattingValue1" then SetChattingVal:InvokeServer(1); task.wait(0.05) end
        PlayerChatted:InvokeServer(args, "Initiate")
        ...
        if c.preSeq == "SetChattingValue1" then task.wait(0.05); SetChattingVal:InvokeServer(1) end
    end
end
...
local function openBoxFor(itemName, teleportDestPos)
    ...
    if not box then
        for _, v in next, workspace:GetDescendants() do
            local iv = v:FindFirstChild("ItemName"); local owner = v:FindFirstChild("Owner")
            if iv and iv.Value == itemName and owner and owner.Value == player then box = v; break end
        end
    end
    ...
    repeat
        ...
        for _, v in next, workspace:GetDescendants() do
            local iv = v:FindFirstChild("ItemName"); local owner = v:FindFirstChild("Owner")
            if iv and iv.Value == itemName and owner and owner.Value == player then found = v; break end
        end
    until found or tick() > deadline
    if not found then return end
    local m = found:FindFirstChild("Main"); if not m then return end
...
local function AB_buy(itemName, amount, isBlueprint, isBatch)
    ...
    if not isBatch then AB_aborted = false; AB_buying = true; setProgress(0, amount); refreshActionButtons() end
    local Dragging = RS.Interaction and RS.Interaction:FindFirstChild("ClientIsDragging")
    local char = player.Character
    if not (char and char:FindFirstChild("HumanoidRootPart")) then
        if not isBatch then AB_buying = false; refreshActionButtons() end; return
    end
    ...
    local function findShopItem()
        for _, store in next, workspace.Stores:GetChildren() do
            ...
            for _, v in next, store:GetChildren() do
                local box = v:FindFirstChild("BoxItemName"); local owner = v:FindFirstChild("Owner")
    ...
    local function waitForShopItem(timeout)
        ...
        while not found and tick() < deadline do
            ...
            task.wait(0.07); found = findShopItem()
        end
        return found
    end
    ...
    for i = 1, amount do
        ...
        hrp.CFrame = main.CFrame + Vector3.new(5, 0, 5); task.wait(0.05)
        for _ = 1, 12 do
            if Dragging then Dragging:FireServer(item) end
            main.CFrame = counterCF + Vector3.new(0, main.Size.Y, 0.5)
            task.wait(0.016)
        end
        hrp.CFrame = counterCF + Vector3.new(5, 0, 5); task.wait(0.05)
        fireDialog(closest)
        local returnStart = tick(); local returned = false
        repeat
            ...
            pcall(function()
                ...
                if isnetworkowner(main) then
                    if Dragging then Dragging:FireServer(item) end
                    main.CFrame = origin; returned = true
                end
            end)
            task.wait(0.05)
        until returned
        ...
        if isBlueprint then task.wait(0.3); openBoxFor(itemName, nil) end
        task.wait(0.2)
        if not isBatch then setProgress(i, amount); setStatus("Bought " .. i .. " / " .. amount, true) end
    end

    if not isBatch then
        AB_buying = false; setProgress(nil)
...
local function mkLabel(text)
    ...
    lbl.Font = Enum.Font.GothamBold; lbl.TextSize = 10; lbl.TextColor3 = C.TEXT_DIM
...
local function mkSep()
    ...
    s.Size = UDim2.new(1, -12, 0, 1); s.BackgroundColor3 = C.BORDER; s.BorderSizePixel = 0
end

local function mkBtn(text, cb)
    local btn = Instance.new("TextButton", autoBuyPage)
    btn.Size = UDim2.new(1, -12, 0, 34); btn.BackgroundColor3 = C.BTN; btn.Text = text
...
local function mkNumberInput(text, minV, maxV, defV, cb)
    ...
    fr.Size = UDim2.new(1, -12, 0, 40); fr.BackgroundColor3 = C.CARD; fr.BorderSizePixel = 0
    Instance.new("UICorner", fr).CornerRadius = UDim.new(0, 6)
    local lbl = Instance.new("TextLabel", fr)
    lbl.Size = UDim2.new(1, -130, 1, 0); lbl.Position = UDim2.new(0, 10, 0, 0)
    lbl.BackgroundTransparency = 1; lbl.Text = text; lbl.Font = Enum.Font.GothamSemibold
    lbl.TextSize = 13; lbl.TextColor3 = C.TEXT; lbl.TextXAlignment = Enum.TextXAlignment.Left
    local function makeArrow(xOff, label)
        ...
        b.BackgroundColor3 = C.BTN; b.Text = label; b.Font = Enum.Font.GothamBold; b.TextSize = 16
        b.TextColor3 = C.TEXT; b.BorderSizePixel = 0; b.AutoButtonColor = false
        Instance.new("UICorner", b).CornerRadius = UDim.new(0, 6)
        local s = Instance.new("UIStroke", b); s.Color = C.BORDER; s.Thickness = 1; s.Transparency = 0
    ...
    local minusBtn = makeArrow(-122, "-"); local plusBtn = makeArrow(-30, "+")
    local box = Instance.new("TextBox", fr)
    box.Size = UDim2.new(0, 56, 0, 28); box.Position = UDim2.new(1, -90, 0.5, -14)
    box.BackgroundColor3 = Color3.fromRGB(22, 22, 22); box.BorderSizePixel = 0
    box.Font = Enum.Font.GothamBold; box.TextSize = 14; box.TextColor3 = C.TEXT
    box.Text = tostring(defV); box.ClearTextOnFocus = false; box.TextXAlignment = Enum.TextXAlignment.Center
    ...
    local function applyVal(v)
        ...
        box.Text = tostring(curVal); if cb then cb(curVal) end
    end
...
statusCard.Size = UDim2.new(1, -12, 0, 38); statusCard.BackgroundColor3 = C.CARD; statusCard.BorderSizePixel = 0
...
statusLbl.Size = UDim2.new(1, -80, 1, 0); statusLbl.Position = UDim2.new(0, 10, 0, 0)
statusLbl.BackgroundTransparency = 1; statusLbl.Font = Enum.Font.GothamSemibold; statusLbl.TextSize = 12
statusLbl.TextColor3 = C.TEXT_DIM; statusLbl.TextXAlignment = Enum.TextXAlignment.Left
statusLbl.Text = "Select an item to get started."
AB_statusLbl = statusLbl
local progLbl = Instance.new("TextLabel", statusCard)
progLbl.Size = UDim2.new(0, 68, 1, 0); progLbl.Position = UDim2.new(1, -72, 0, 0)
progLbl.BackgroundTransparency = 1; progLbl.Font = Enum.Font.GothamBold; progLbl.TextSize = 12
progLbl.TextColor3 = C.TEXT; progLbl.TextXAlignment = Enum.TextXAlignment.Right; progLbl.Visible = false
AB_progLbl = progLbl

-- ITEM DROPDOWN
mkSep(); mkLabel("Item")

local ITEM_H   = 30; local MAX_SHOW = 6; local HEADER_H = 38
local dropIsOpen = false; local dropSelected = ""; local dropItems = {}

local dropOuter = Instance.new("Frame", autoBuyPage)
dropOuter.Size = UDim2.new(1, -12, 0, HEADER_H); dropOuter.BackgroundColor3 = C.BG_ROW
dropOuter.BorderSizePixel = 0; dropOuter.ClipsDescendants = true
Instance.new("UICorner", dropOuter).CornerRadius = UDim.new(0, 7)
local dropOuterStroke = Instance.new("UIStroke", dropOuter)
dropOuterStroke.Color = C.BORDER; dropOuterStroke.Thickness = 1; dropOuterStroke.Transparency = 0.3

local dropHeader = Instance.new("Frame", dropOuter)
dropHeader.Size = UDim2.new(1, 0, 0, HEADER_H); dropHeader.BackgroundTransparency = 1

local dropLabelLeft = Instance.new("TextLabel", dropHeader)
dropLabelLeft.Size = UDim2.new(0, 50, 1, 0); dropLabelLeft.Position = UDim2.new(0, 10, 0, 0)
dropLabelLeft.BackgroundTransparency = 1; dropLabelLeft.Text = "Item"
dropLabelLeft.Font = Enum.Font.GothamBold; dropLabelLeft.TextSize = 11
dropLabelLeft.TextColor3 = C.TEXT_DIM; dropLabelLeft.TextXAlignment = Enum.TextXAlignment.Left

local selFrame = Instance.new("Frame", dropHeader)
selFrame.Size = UDim2.new(1, -66, 0, 26); selFrame.Position = UDim2.new(0, 58, 0.5, -13)
selFrame.BackgroundColor3 = C.BG_INPUT; selFrame.BorderSizePixel = 0
...
selLbl.Size = UDim2.new(1, -30, 1, 0); selLbl.Position = UDim2.new(0, 8, 0, 0)
selLbl.BackgroundTransparency = 1; selLbl.Text = "Select item..."
selLbl.Font = Enum.Font.GothamSemibold; selLbl.TextSize = 11
...
divider.Size = UDim2.new(1, -14, 0, 1); divider.Position = UDim2.new(0, 7, 0, HEADER_H)
...
listScroll.ScrollBarThickness = 3; listScroll.ScrollBarImageColor3 = C.BORDER_FOC
...
listPad.PaddingLeft = UDim.new(0, 5); listPad.PaddingRight = UDim.new(0, 5)

local AB_item_ref, AB_isBlueprint_ref = nil, false

local function applySelection(entry)
    dropSelected = entry.name; AB_item = entry.name; AB_isBlueprint = entry.isBlueprint
    local priceStr = entry.price > 0 and ("   $" .. entry.price) or ""
    selLbl.Text = entry.name .. priceStr; selLbl.TextColor3 = C.TEXT_WHITE
    arrowLbl.TextColor3 = C.TEXT_MID; dropOuterStroke.Color = C.BORDER_FOC
    refreshActionButtons()
end

local function clearSelection()
    dropSelected = ""; AB_item = nil; AB_isBlueprint = false
    selLbl.Text = "Select item..."; selLbl.TextColor3 = C.TEXT_DIM
    arrowLbl.TextColor3 = C.TEXT_DIM; dropOuterStroke.Color = C.BORDER
    refreshActionButtons()
end
...
local function buildList()
    ...
    for i, entry in ipairs(dropItems) do
        ...
        row.Size = UDim2.new(1, 0, 0, ITEM_H); row.BackgroundColor3 = isSel and Color3.fromRGB(50,50,50) or C.BG_ROW
        row.BorderSizePixel = 0; row.LayoutOrder = i
        Instance.new("UICorner", row).CornerRadius = UDim.new(0, 5)
        local nameLbl = Instance.new("TextLabel", row)
        nameLbl.Size = UDim2.new(1, -70, 1, 0); nameLbl.Position = UDim2.new(0, 10, 0, 0)
        nameLbl.BackgroundTransparency = 1; nameLbl.Text = entry.name
        nameLbl.Font = Enum.Font.GothamSemibold; nameLbl.TextSize = 11
        nameLbl.TextColor3 = C.TEXT_WHITE; nameLbl.TextXAlignment = Enum.TextXAlignment.Left
        ...
        if entry.price > 0 then
            ...
            priceLbl.BackgroundTransparency = 1; priceLbl.Text = "$" .. entry.price
            priceLbl.Font = Enum.Font.Gotham; priceLbl.TextSize = 11
            priceLbl.TextColor3 = C.TEXT_WHITE; priceLbl.TextXAlignment = Enum.TextXAlignment.Right
        end
        ...
        rowBtn.MouseEnter:Connect(function()
            if entry.name ~= dropSelected then tw(row, { BackgroundColor3 = Color3.fromRGB(36, 36, 36) }) end
        end)
        rowBtn.MouseLeave:Connect(function()
            if entry.name ~= dropSelected then tw(row, { BackgroundColor3 = C.BG_ROW }) end
        end)
        rowBtn.MouseButton1Click:Connect(function()
            if entry.name == dropSelected then clearSelection() else applySelection(entry) end
            buildList(); task.delay(0.04, closeList)
        end)
    end
end
...
headerBtn.MouseButton1Click:Connect(function() if dropIsOpen then closeList() else openList() end end)
headerBtn.MouseEnter:Connect(function() tw(selFrame, { BackgroundColor3 = Color3.fromRGB(40, 40, 40) }) end)
headerBtn.MouseLeave:Connect(function() tw(selFrame, { BackgroundColor3 = C.BG_INPUT }) end)

task.spawn(function() task.wait(0.8); dropItems = grabAllItems() end)

-- OPTIONS
mkSep(); mkLabel("Options")
...
actionRow.Size = UDim2.new(1, -12, 0, 36); actionRow.BackgroundTransparency = 1

local startBtn = Instance.new("TextButton", actionRow)
startBtn.Size = UDim2.new(0.5, -4, 1, 0); startBtn.Position = UDim2.new(0, 0, 0, 0)
startBtn.BackgroundColor3 = C.BTN; startBtn.Text = "Start"
startBtn.Font = Enum.Font.GothamBold; startBtn.TextSize = 13
startBtn.TextColor3 = C.TEXT_DIM; startBtn.BorderSizePixel = 0; startBtn.AutoButtonColor = false
...
stopBtn.Size = UDim2.new(0.5, -4, 1, 0); stopBtn.Position = UDim2.new(0.5, 4, 0, 0)
stopBtn.BackgroundColor3 = C.BTN; stopBtn.Text = "Stop"
stopBtn.Font = Enum.Font.GothamBold; stopBtn.TextSize = 13
stopBtn.TextColor3 = C.TEXT_DIM; stopBtn.BorderSizePixel = 0; stopBtn.AutoButtonColor = false
...
AB_startBtn = startBtn; AB_stopBtn = stopBtn; refreshActionButtons()

startBtn.MouseButton1Click:Connect(function()
    if AB_buying or not AB_item then return end
    task.spawn(function() AB_buy(AB_item, AB_amount, AB_isBlueprint, false) end)
end)

stopBtn.MouseButton1Click:Connect(function()
    if not AB_buying then return end
    AB_aborted = true; setStatus("Stopping...", true)
end)

-- SPECIAL
mkSep(); mkLabel("Special")

mkBtn("Buy All Blueprints", function()
    if AB_buying then return end
    task.spawn(function()
        AB_buying = true; AB_aborted = false; refreshActionButtons()
        local bps = grabBlueprintNames()
        if #bps == 0 then setStatus("No blueprints found in stores.", false); AB_buying = false; refreshActionButtons(); return end
        for i, bp in ipairs(bps) do
            if AB_aborted then break end
            setStatus("[" .. i .. "/" .. #bps .. "]  " .. bp, true)
            AB_buy(bp, 1, true, true)
            if not AB_aborted then task.wait(0.3); openBoxFor(bp, nil) end
        end
        AB_buying = false; setProgress(nil)
    ...
    task.spawn(function()
        ...
        local function openThenTeleport(itemName, destPos)
            ...
            pcall(function()
                ...
                local t = 0
                while not isnetworkowner(m) and t < 3 do
                    if Dragging then Dragging:FireServer(found) end; task.wait(0.05); t += 0.05
                end
                if Dragging then Dragging:FireServer(found) end
        ...
        if axe then
            ...
            if ConfirmIdentity then
                ConfirmIdentity:InvokeServer(RS:FindFirstChild("Tool", true) or workspace:FindFirstChild("Tool", true), "Rukiryaxe")
            end
            task.wait(0.243)
        ...
        AB_buying = false; refreshActionButtons()
    end)
end)

-- SERVICES
mkSep(); mkLabel("Services  (pay at counter)")
...
for _, svc in ipairs(AB_Services) do
    ...
    btn.MouseButton1Click:Connect(function()
        task.spawn(function()
            ...
            PC:InvokeServer(args, "Initiate"); task.wait(0.05); SCV:InvokeServer(2)
            task.wait(svc.wConfirm)
            PC:InvokeServer(args, "ConfirmPurchase"); task.wait(0.05); SCV:InvokeServer(2)
            task.wait(svc.wEnd)
...
-- CLEANUP
table.insert(cleanupTasks, function() AB_aborted = true; AB_buying = false end)
